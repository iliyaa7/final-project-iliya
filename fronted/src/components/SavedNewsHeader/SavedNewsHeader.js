...
function SavedNewsHeader(props) {
  ...
  return(
    <div className={`saved-news__header-container ${(props.savedArticles.length === 0) && 'saved-news__header-container_long'}`}>
      ...
      <h3 className='saved-news__discription'>{}{props.savedArticles.length > 0 ?  `, you have ${props.savedArticles.length} ${props.savedArticles.length < 2 ? 'saved article' : 'saved articles'}` : ", you haven't saved any articles yet"} </h3>
